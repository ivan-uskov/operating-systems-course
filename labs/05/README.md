...
- `add_file PATH`. Индексирует файл с путём `PATH`.
  Повторная индексация одного и того же документа считается ошибкой.
- `add_dir PATH`. Индексирует все файлы внутри директории `PATH`. Индексирование файлов выполняется параллельно
